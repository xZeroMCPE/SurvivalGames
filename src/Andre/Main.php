...
{	
	...
	public function onEnable() {
        ...
        && !($this->money = $pm->getPlugin("PocketMoney")))			{
			$this->getServer()->getLogger()->info(TextFormat::RED. "[SG] Please Install EconomyAPI For This Plugin To Work Or PocketMoney!!!");
		} else {
	...
	public function onCommand(CommandSender $sender, Command $command, $label, array $args)
	{
		if($command->getName()=="lobby")
		{
			...
			{
				$sender->sendMessage("[{$this->getConfig()->get("prefix")}] Sorry, The match already started. Please leave server to quit arena");
				return;
			}
			...
			{	
				...
				if($this->gameStatus==1 && count($this->players)<2)
				{
					$this->gameStatus=0;
					$this->lastTime=0;
					$event->getPlayer()->sendMessage("[{$this->getConfig()->get("prefix")}] Tournament, will start soon");
		...
		case "set":
			if($this->config->exists("lastpos"))
			{
				$sender->sendMessage(TextFormat::RED. "[{$this->getConfig()->get("prefix")}]The game was set before. Please use /sg remove and try again.");
			}
			else
			{
				$name=$sender->getName();
		...
		case "remove":
			...
			break;
		case "start":
			Server::getInstance()->broadcastMessage(TextFormat::BLUE. "[Game] Tournament was force start.");
			$this->gameStatus=1;
			$this->lastTime=5;
			break;
		case "reload":
	...
	public function onPlayerCommand(PlayerCommandPreprocessEvent $event)
	{
		...
		default:
			$event->setCancelled();
			$event->getPlayer()->sendMessage("[{$this->getConfig()->get("prefix")}] Sorry, There was an error. ");
			$event->getPlayer()->sendMessage("[{$this->getConfig()->get("prefix")}] Leave the server to quit match");
			break;
		}
		unset($event);
	...
	public function gameTimber(){
		...
		if($this->gameStatus==3 || $this->gameStatus==4)
		{
			...
			{
				Server::getInstance()->broadcastMessage("[{$this->getConfig()->get("prefix")}] Arena: SG-1 is now accepting players.");
				$this->gameStatus=0;
				$this->lastTime=0;
				$this->clearChest();
				$this->ClearAllInv();
			...
			if(isset($this->lastpos) && $this->lastpos!=array() && $sign instanceof Sign && $sign->getX()==$this->sign->x && $sign->getY()==$this->sign->y && $sign->getZ()==$this->sign->z && $event->getPlayer()->getLevel()->getFolderName()==$this->config->get("sign")["level"])
			{
				...
				if($this->gameStatus==0 || $this->gameStatus==1)
				{
					if(!isset($this->players[$event->getPlayer()->getName()]))
					{
						...
						{
							$event->getPlayer()->sendMessage("[{$this->getConfig()->get("prefix")}] This match is full, Please wait wait or join a diffrent survival games match!.");
							return;
						}
	...
	public function onDisable(){
		$this->config->save();
        	$this->points->save();
			$this->getServer()->getLogger()->info(TextFormat::GREEN."[sg] Saved All Configs/Data... Disabling....");
	}
}
?>
